...
|  `ConnectionErrorCount`  |  The total error count for all types of MAC level errors on the AWS device\. The total includes cyclic redundancy check \(CRC\) errors\. You can use the sum statistic for this metric\. For more information about getting the sum statistic, see [Getting Statistics for a Metric](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/getting-metric-statistics.html) in the *Amazon CloudWatch User Guide*\. This count replaces `ConnectionCRCErrorCount`, which is no longer in use\. Units: Count  | 
