...
After you enable jumbo frames for private virtual interface or transit virtual interface, you can only associate it with a connection or LAG that is jumbo frame capable\. Jumbo frames are supported on private virtual interfaces that are attached to a virtual private gateway or a Direct Connect gateway, and transit virtual interfaces that are attached to a Direct Connect gateway\. If you add static routes to your virtual private gateway or transit gateway, traffic that is routed through the static route defaults to 1500 MTU\. If there are two private virtual interfaces or transit virtual interfaces that advertise the same route, but use different MTU values, the MTU value that is used for the traffic is 1500 MTU\.

**Important**  
Jumbo frames apply only to propagated routes from AWS Direct Connect\. If you add static routes to your virtual private gateway or transit gateway, then traffic routed through the static routes is sent using 1500 MTU\.  
